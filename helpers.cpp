...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	//calculate sum of values in grid
	float total = 0;
	...
	//for all cells in grid, divide by total and append to newGrid
	vector<float> row;
	for (int i = 0; i < grid.size(); i++) {
		row.clear();
		for (int j = 0; j < grid[0].size(); j++)
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	//initialize newGrid used for blurring
	int height = grid.size();
	int width = grid[0].size();
	vector < vector <float> > newGrid = zeros(height,width);
	
	//initialize blurring window
	...
	//apply blurring and put blurred values into newGrid
	int blur_y, blur_x;
	float blur_value;
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++)
		{
			for (int dy = 0; dy < 3; dy++)
			{
				...
				{
					blur_value = window[dy][dx];
					blur_y = (i + dy - 1) % height;
					if(blur_y == -1) blur_y = height - 1;
					blur_x = (j + dx - 1) % width;
					if(blur_x == -1) blur_x = width - 1;
					newGrid[blur_y][blur_x] += grid[i][j]*blur_value;
				}
			}
		}
	}
